...
stock_data = {
    "RELIANCE": {"name": "Reliance Industries", "price": 2750.50, "volatility": 0.02},
    "TCS": {"name": "Tata Consultancy Services", "price": 3850.75, "volatility": 0.015},
    "INFY": {"name": "Infosys", "price": 1650.25, "volatility": 0.018},
    "HDFCBANK": {"name": "HDFC Bank", "price": 1650.80, "volatility": 0.016},
    "ICICIBANK": {"name": "ICICI Bank", "price": 1050.60, "volatility": 0.019},
    "NIFTYBEES": {"name": "Nifty ETF", "price": 220.45, "volatility": 0.012},
    "GOLDBEES": {"name": "Gold ETF", "price": 55.30, "volatility": 0.008},
}

# Function to simulate price changes
def update_prices():
    for ticker, data in stock_data.items():
        change = np.random.normal(0, data["volatility"])
        stock_data[ticker]["price"] = max(data["price"] * (1 + change), data["price"] * 0.8)
...
# Mock Portfolio Page
        ...
        if st.session_state.mock_portfolio:
            ...
            for ticker, details in st.session_state.mock_portfolio.items():
                if ticker in stock_data:
                    ...
                    total_invested += details["invested_amount"]
                    total_current += current_value
                    
                    st.markdown(f"""
                    <div class='portfolio-item'>
                        <h4>{ticker} - {stock_data[ticker]['name']}</h4>
                        <p>Quantity: {details["quantity"]} | Avg Price: ₹{details["avg_price"]:.2f}</p>
                        <p>Current Price: ₹{current_price:.2f} | Invested: ₹{details["invested_amount"]:.2f}</p>
                        <p>Current Value: ₹{current_value:.2f} | P/L: ₹{profit_loss:.2f} ({profit_loss_pct:.2f}%)</p>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.markdown(f"""
                    <div class='portfolio-item'>
                        <h4>{ticker}</h4>
                        <p>Quantity: {details["quantity"]} | Avg Price: ₹{details["avg_price"]:.2f}</p>
                        <p>Error: Stock data not available</p>
                    </div>
                    """, unsafe_allow_html=True)
            
            # Update portfolio value
            cash = st.session_state.portfolio_value - total_invested
            st.session_state.portfolio_value = total_current + cash
            
            st.subheader("Portfolio Summary")
            st.markdown(f"""
            <div class='portfolio-item'>
                <p>Total Invested: ₹{total_invested:.2f}</p>
                <p>Current Value: ₹{total_current:.2f}</p>
                <p>Cash Balance: ₹{cash:.2f}</p>
                <p>Total Portfolio Value: ₹{st.session_state.portfolio_value:.2f}</p>
            </div>
            """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        with st.form("add_investment"):
            ...
            if asset_type == "Stock":
                ticker = st.selectbox("Select Stock", list(stock_data.keys())[:5])  # First 5 are stocks
            else:
                ticker = st.selectbox("Select ETF", list(stock_data.keys())[5:])  # Last 2 are ETFs
            ...
            if submitted:
                if ticker:
                    try:
                        ...
                        st.error("Could not process investment for {}. Please try again.".format(ticker))
                else:
                    st.error("Please select a valid investment")
        
        st.subheader("Sell Investment")
        if st.session_state.mock_portfolio:
            with st.form("sell_investment"):
                ticker_to_sell = st.selectbox("Select investment to sell", list(st.session_state.mock_portfolio.keys()))
                quantity_to_sell = st.number_input("Quantity to sell", min_value=1, 
                                                  max_value=st.session_state.mock_portfolio[ticker_to_sell]["quantity"])
                
                submitted_sell = st.form_submit_button("Sell")
                
                if submitted_sell:
                    try:
                        if ticker_to_sell in stock_data:
                            current_price = stock_data[ticker_to_sell]["price"]
                        else:
                            # Use average price if stock data not available
                            current_price = st.session_state.mock_portfolio[ticker_to_sell]["avg_price"]
                        
                        sale_value = current_price * quantity_to_sell
                        investment = st.session_state.mock_portfolio[ticker_to_sell]
                        
                        if quantity_to_sell == investment["quantity"]:
                            # Sell all shares
                            del st.session_state.mock_portfolio[ticker_to_sell]
                        else:
                            # Sell partial holding
                            new_quantity = investment["quantity"] - quantity_to_sell
                            new_invested = investment["invested_amount"] * (new_quantity / investment["quantity"])
                            
                            st.session_state.mock_portfolio[ticker_to_sell] = {
                                "quantity": new_quantity,
                                "avg_price": new_invested / new_quantity,
                                "invested_amount": new_invested
                            }
                        
                        # Add sale proceeds to cash
                        st.session_state.portfolio_value += sale_value
                        st.success("Sold {} shares of {}".format(quantity_to_sell, ticker_to_sell))
                        st.experimental_rerun()
                    
                    except:
                        st.error("Could not process sale for {}. Please try again.".format(ticker_to_sell))
    ...
    if st.session_state.mock_portfolio:
        ...
        for ticker, details in st.session_state.mock_portfolio.items():
            if ticker in stock_data:
                ...
                allocation_data.append({"Asset": f"{ticker} ({stock_data[ticker]['name']})", "Value": current_value, "Percentage": 0})
            else:
                # For items not in our stock data
                current_value = details["avg_price"] * details["quantity"]
                total_value += current_value
                allocation_data.append({"Asset": ticker, "Value": current_value, "Percentage": 0})
        
        # Add cash position
        cash = st.session_state.portfolio_value - total_value
        allocation_data.append({"Asset": "Cash", "Value": cash, "Percentage": 0})
        total_value += cash
        ...
        - Detailed analytics on your portfolio
